...
public class Unity3dIoCContainer : IIoCContainer, IServiceLocator, IDependencyInjector
{
    ...
    /// <summary>
    /// The singleton instances of monobehaviours
    /// </summary>
    readonly Dictionary<Type, object> mMonoBehaviourSingletons = new Dictionary<Type, object>();
    ...
    public object Resolve(Type type, string key = null)
    {
        ...
        if (foundType.IsSubclassOf(typeof(MonoBehaviour))) // this is the unity3d specific part
        {
            ...
            var go = GameObject.Find(this.singletonGameObjectName)
                ?? new GameObject(this.singletonGameObjectName);

            if (!mMonoBehaviourSingletons.ContainsKey(foundType))
            {
                var existingMonoBehaviours = Object.FindObjectsOfType(foundType);
                Guard(existingMonoBehaviours.Length > 1, "More than one type of {0} found in scene. Unable to use as singleton for IoC.", foundType.Name);

                mMonoBehaviourSingletons.Add(foundType,
                    existingMonoBehaviours.Length == 1
                        ? existingMonoBehaviours.Single()
                        : go.AddComponent(foundType));

                Inject(foundType, mMonoBehaviourSingletons[foundType]);
            }
            
           
            return mMonoBehaviourSingletons[foundType];
        }
